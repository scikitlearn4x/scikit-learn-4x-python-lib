...
class TestBinaryBuffer(TestCase):
    ...
    def test_append_null_array_of_string(self):
        buffer = BinaryBuffer()
        buffer.append_array_of_string(None)
    ...
    def test_append_array_of_string(self):
        buffer = BinaryBuffer()

        array = ['a', 'b', None]

        buffer.append_array_of_string(array)
        data = buffer.to_buffer()
        expected = [1, 3, 0, 0, 0, 1, 1, 0, 0, 0, 97, 1, 1, 0, 0, 0, 98, 0]

        test_array_similarity(self, data, expected)

    def test_append_array_of_string_in_dictionary(self):
        buffer = BinaryBuffer()

        dic = {'str_array': ['a', 'b']}

        buffer.append_dictionary(dic)
        data = buffer.to_buffer()
        expected = [1, 1, 0, 0, 0, 1, 9, 0, 0, 0, 115, 116, 114, 95, 97, 114, 114, 97, 121, 67, 1, 2, 0, 0, 0, 1, 1, 0, 0, 0, 97, 1, 1, 0, 0, 0, 98]
